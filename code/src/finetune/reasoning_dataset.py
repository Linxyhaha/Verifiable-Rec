...
class RecDataset(Dataset):
    def __init__(self, train_file, asin2catid_file, tokenizer, max_len=2048, sample=-1, test = False, seed=0, category="", K=4, dedup=False):
        self.data = pd.read_csv(train_file)
        self.asin2catid = np.load(asin2catid_file, allow_pickle=True).item()
        random.seed(seed)
...
{data_point["output"]}"""
    ...
    def get_history(self, row):
        ...
        # ipdb.set_trace()
        target_asin = str(row["item_asin"])
        category = self.asin2catid[target_asin]
...
"""
        ...
        return {
            ...
            "prefix_end_id": len(tokens)-1,
            "category_label": history["category"],
        }
...
class LatentRDataset_Pretrain(Dataset):
    '''
        add the <thought> token
    '''
    def __init__(self, train_file, asin2catid_file, asin2clusterid_file, tokenizer, max_len=2048, sample=-1, test = False, seed=0, category="", K=4, dedup=False):
        self.data = pd.read_csv(train_file)
        self.asin2catid = np.load(asin2catid_file, allow_pickle=True).item()

        try:
            self.asin2clusterid = np.load(asin2clusterid_file, allow_pickle=True).item()
        except:
            print("no cluster id file found")
            pass
        random.seed(seed)
...
{data_point["output"]}"""
    ...
    def get_history(self, row, sample_idx):
        row['history_item_title'] = eval(row['history_item_title'])
        L = len(row['history_item_title']) 
        history = ""
        for i in range(L):
        ...
        target_asin = str(row["item_asin"])
        category = self.asin2catid[target_asin]
        cluster = self.asin2clusterid[target_asin] 
        category_label = [category, cluster]
