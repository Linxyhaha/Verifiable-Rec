...
class SelfAttentionLayer(torch.nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor,
        ...
        # hidden state (bs, seq_len, dim), attention_mask (bs, seq_len), thought_id_idx (bs,) - column idx
...
class LatentModelwithVerifier_RATT(Qwen2ForCausalLM):
    '''
        This model has the additional RATT 
        workflow: reasoning then generate
        training: reasoning loss is verifier loss, generation loss is MLE (the latentR3 baseline)
        this is for the pre-training
    '''
    def __init__(self, config):
        super().__init__(config)
    ...
    def generate_embs(self, input_ids, input_embeds, attention_mask):
        ...
        input_embs[where_thought_ids[:, 0], where_thought_ids[:, 1]] = thought_embeds
        return input_embs, where_thought_ids, thought_embeds

    def cal_consistency(self):
        pass 

    def freeze_llm_params(self):
        # freeze llm and RATT params
